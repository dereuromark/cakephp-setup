...
class MaxInputVarsCheck extends Check {
	...
	/**
	 * Default recommended minimum for apps with complex forms.
	 *
	 * @var int
	 */
	...
	/**
	 * @param int|null $requiredMin
	 */
	public function __construct(?int $requiredMin = null) {
		$this->maxInputVars = (int)ini_get('max_input_vars');
		$this->requiredMin = $requiredMin ?? static::RECOMMENDED_MIN;
	}
	...
	public function check(): void {
		$this->passed = $this->maxInputVars >= $this->requiredMin;
	...
	protected function addFixInstructions(): void {
		...
		if ($phpIniPath) {
			...
			$this->infoMessage[] = 'Quick fix: `sudo sed -i \'s/^;\\?max_input_vars.*/max_input_vars = ' . $this->requiredMin . '/\' ' . $phpIniPath . '`';
		}
